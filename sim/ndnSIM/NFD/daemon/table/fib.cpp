...
void
Fib::addOrUpdateNextHop(Entry& entry, Face& face, uint64_t endpointId, uint64_t cost)
{
  NextHopList::iterator it;
  bool isNew;
  std::tie(it, isNew) = entry.addOrUpdateNextHop(face, endpointId, cost);
...
Fib::removeNextHop(Entry& entry, const Face& face, uint64_t endpointId)
{
  bool isRemoved = entry.removeNextHop(face, endpointId);
