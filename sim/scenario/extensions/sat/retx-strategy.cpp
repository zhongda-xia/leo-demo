...
NFD_REGISTER_STRATEGY(RetxStrategy);

NFD_LOG_INIT(RetxStrategy);

const time::milliseconds RetxStrategy::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds RetxStrategy::RETX_SUPPRESSION_MAX(250);

RetxStrategy::RetxStrategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
                      ...
                      RETX_SUPPRESSION_MAX)
{
  ...
  if (!parsed.parameters.empty()) {
    BOOST_THROW_EXCEPTION(std::invalid_argument("RetxStrategy does not accept parameters"));
  }
  if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
    BOOST_THROW_EXCEPTION(std::invalid_argument(
      "RetxStrategy does not support version " + to_string(*parsed.version)));
  }
...
const Name&
RetxStrategy::getStrategyName()
{
  static Name strategyName("/localhost/nfd/strategy/retx/%FD%01");
  return strategyName;
}

void
RetxStrategy::afterReceiveInterest(const Face& inFace, const Interest& interest,
...
bool
RetxStrategy::isNextHopEligible(const Face& inFace, const Interest& interest,
...
void
RetxStrategy::afterNewNextHop(const fib::NextHop& nextHop,
                              const shared_ptr<pit::Entry>& pitEntry)
{
  // no need to check for suppression, as it is a new next hop

  NFD_LOG_DEBUG("New NextHop!");
