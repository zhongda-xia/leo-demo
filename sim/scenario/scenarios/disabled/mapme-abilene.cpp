...
NS_LOG_COMPONENT_DEFINE("mapme.abilene");
...
main(int argc, char* argv[])
{
  ...
  auto strawmanNode = CreateObject<Node>();
  nodes.Add(strawmanNode); // strawman
  int strawmanAttachId = 4;
  p2p.Install(strawmanNode, nodes.Get(strawmanAttachId)); // strawman <-> ?
  ...
  consumerApps.Start(Seconds(20)); // delay start
  ...
  mobileProducerApps.Start(Seconds(10.0)); // delay start
  ...
  int i = 12;
  getNodeInfo(NodeList::GetNode(i));
