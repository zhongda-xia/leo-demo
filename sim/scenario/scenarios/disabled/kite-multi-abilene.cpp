...
NS_LOG_COMPONENT_DEFINE("kite.multi.abilene");
...
main(int argc, char* argv[])
{
  ...
  topologyReader.SetFileName("topologies/topo-abilene-mod.txt");
  nodes = topologyReader.Read();

  NodeContainer customNodes;
  customNodes.Create(4); // consumer, RV1, RV2, server

  PointToPointHelper p2p;
  p2p.Install(customNodes.Get(0), nodes.Get(0)); // consumer <-> ?
  p2p.Install(customNodes.Get(1), nodes.Get(10)); // rv1 <-> 1
  p2p.Install(customNodes.Get(2), nodes.Get(7)); // rv2 <-> 1
  p2p.Install(customNodes.Get(3), nodes.Get(5)); // server <-> 1
  ...
  posAlloc->Add(Vector(400.0, 100.0, 0.0));
  posAlloc->Add(Vector(400.0, 200.0, 0.0));
  posAlloc->Add(Vector(400.0, 300.0, 0.0));
  ...
  y->SetAttribute("Max", DoubleValue(-50));
  ...
  mobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel", "Bounds",
                            RectangleValue(Rectangle(-200, 200, -200, -50)), "Distance",
                            DoubleValue(200), "Speed", StringValue(ss.str()));

  // Make mobile nodes move
  mobility.Install(mobileNodes);

  // Setup fixed position of mobile node
  ...
  ndn::StrategyChoiceHelper::InstallAll(rvPrefix, string("/localhost/nfd/strategy/best-route"));
  ndn::StrategyChoiceHelper::InstallAll(rvPrefix + producerSuffix, string("/localhost/nfd/strategy/") + strategy);

  std::string serverPrefix = "/server";

  ndn::GlobalRoutingHelper ndnGlobalRoutingHelper;
  ndnGlobalRoutingHelper.Install(nodes);
  ndnGlobalRoutingHelper.Install(customNodes);

  ndnGlobalRoutingHelper.AddOrigins(rvPrefix, customNodes.Get(1));
  ndnGlobalRoutingHelper.AddOrigins("/global/1", customNodes.Get(1));
  ndnGlobalRoutingHelper.AddOrigins(rvPrefix, customNodes.Get(2));
  ndnGlobalRoutingHelper.AddOrigins("/global/2", customNodes.Get(2));
  ndnGlobalRoutingHelper.AddOrigins(serverPrefix, customNodes.Get(3));

  // set producer region
  nodes.Get(10)->GetObject<ndn::L3Protocol>()->getForwarder()->getNetworkRegionTable().insert("/global/1");
  nodes.Get(7)->GetObject<ndn::L3Protocol>()->getForwarder()->getNetworkRegionTable().insert("/global/2");

  // Installing applications

  // Consumer
  int consumerNodeId = 0;
  ...
  ApplicationContainer consumerApps = consumerHelper.Install(customNodes.Get(consumerNodeId)); // consumer
  ...
  rvHelper.SetAttribute("ServerPrefix", StringValue(serverPrefix));
  rvHelper.SetAttribute("GlobalPrefix", StringValue("/global/1"));
  rvHelper.Install(customNodes.Get(1));

  // ndn::AppHelper rvHelper("ns3::ndn::kite::Rv");
  rvHelper.SetAttribute("Prefix", StringValue(rvPrefix));
  rvHelper.SetAttribute("Prefix", StringValue(rvPrefix));
  rvHelper.SetAttribute("ServerPrefix", StringValue(serverPrefix));
  rvHelper.SetAttribute("GlobalPrefix", StringValue("/global/2"));
  rvHelper.Install(customNodes.Get(2));

  // server
  ndn::AppHelper serverHelper("ns3::ndn::mapping::Server");
  serverHelper.SetAttribute("Prefix", StringValue(serverPrefix));
  serverHelper.Install(customNodes.Get(3));
  ...
  ndn::AppDelayTracer::Install(customNodes.Get(consumerNodeId),
